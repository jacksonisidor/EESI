"""Flora — Self-contained model definition for inference.

Flora is a fine-tuned DFN-CLIP ViT-H/14-378 visual encoder with LoRA adapters
and a learned projection head. It produces L2-normalised 512-dim embeddings
suitable for logo/image retrieval via cosine similarity (dot product).
...
def load_flora(
    checkpoint_path: str,
    device: str = "cpu",
) -> tuple[Flora, torch.nn.Module]:
    """Load a Flora model from a checkpoint.
